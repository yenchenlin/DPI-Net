...
if args.resume_epoch > 0 or args.resume_iter > 0:
    if args.pruning_perc == 0:
        model_path = os.path.join(args.outf, 'net_epoch_%d_iter_%d.pth' % (args.resume_epoch, args.resume_iter))
    else:
        model_path = os.path.join(args.outf, 'net_epoch_%d_iter_%d_pruning_%d.pth' % (args.resume_epoch, args.resume_iter, args.pruning_perc))
        ...
        for i, data in enumerate(dataloaders[phase]):
            ...
            if i % args.log_per_iter == 0:
                ...
                for p in model.parameters():
                    if p.requires_grad:
                        num_params += p.numel()
                        num_zeros += np.sum((p == 0).cpu().numpy())
                ...
                # Log
                print('%s [%d/%d][%d/%d] n_relations: %d, Loss: %.6f, Agg: %.6f, Sparsity: %.6f' %
                      (phase, epoch, args.n_epoch, i, len(dataloaders[phase]),
                       n_relations, np.sqrt(loss.item()), losses / (i + 1), sparsity))
                num_params = 0
                num_zeros = 0
            ...
            if phase == 'train' and i > 0 and i % args.ckp_per_iter == 0:
                if args.pruning_perc == 0:
                    ...
                    torch.save(model.state_dict(), '%s/net_epoch_%d_finetune_iter_%d_pruning_%d.pth' % (args.outf, epoch, i, args.pruning_perc))
